...
상세는 [07](07-react-tauri-migration.md)(프론트 전환), [08](08-shared-analysis-and-timeline.md)(공유 분석·타임라인) 참고. docs 01~06은 구(舊) Flutter 기준이라 프론트 서술은 outdated.
...
- **`users.password_hash`를 NULL로 두지 말 것** — 로그인도 설정창도 아닌 상태 유발 가능(과거 버그). 현재 `sh.cho@llsollu.com`은 테스트로 NULL 상태 → 다음 로그인 시 비번 설정창이 정상 동작으로 뜸.
